...
class Finance extends CI_Controller {
  ...
  public function addObject( $name, $object ) {
    Finance::$mapping[$name] = $object;
  }

  public function object( $name ) {
    if ( isset( Finance::$mapping[$name] ) ) {
      return Finance::$mapping[$name];
    }
    return false;
  }
  ...
  public function table( &$join = array() ) {
    $vars = get_class_vars( get_class( $this ) );
    $table = Finance::userName() . "_" . $vars['table'];
  ...
  public function field ( $name = 'id', $comment = '' ) {
    ...
    return Finance::define_field( $name, $type, $comment, $default, $options );
  }
  ...
  public function connect() {
    ...
    global $db_access;
    Finance::$link = mysql_connect( $db_access['host'], 
				    $db_access['username'], 
				    $db_access['passwd'] );
    if ( Finance::$link ) {
  ...
  public static function prefixColumns( &$columns, $name = '' ) {
    ...
    foreach ( array_keys( $columns ) as $column ) {
      $source = $columns[$column];
      if ( Finance::isWord( $source ) ) {
	$columns[$column] = Finance::prefixName( $source, $name );
      }
    }
  }
  ...
  public function mergeWith( $other, &$columns = array(), $table = '' ) {    
    ...
    Finance::prefixColumns( $columns, $name );
    ...
    Finance::simplifyColumns( $columns );
  ...
  public static function style( $name = 'font_size' ) {
    $blackberry = Finance::isBlackberry();
    switch ($name) {
    case 'font_size':
  ...
  public function filter( $input, &$values = false, &$settings = array() ) {
    ...
    return Finance::filterMatchValues( $input, $values, true );
  }
  ...
  protected function filterMatch( $input, $value, &$option ) {
    if ( $input == '' ) {
      ...
      $option = $match[1] . Finance::highlight( $match[2] ) . $match[3];       
      return true;
    }
    return false;
  }  
